...
class ColliderHourglass : public Collider
{
private:
    float height_half = 1.0f; // 2.0f / 2.0f
    float radius_max = 0.6f;
    float radius_neck = 0.05f;

public:
    ColliderHourglass() {}
    virtual bool isInside(const Particle* p) const;
    virtual bool testCollision(const Particle* p, Collision& colInfo) const;
    // resolveCollision is generic for all colliders
    // virtual void resolveCollision(Particle* p, const Collision& colInfo, double kBounce, double kFriction) override;
};
